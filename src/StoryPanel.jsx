...
const isTextReference = resource => Object.hasOwn(resource, 'thumbnail') && !resource.thumbnail && !resource.mediaSrc
const icons={主体:UserRound,人物:UserRound,服饰:Shirt,场景:Image,产品:Box,道具:Box,视频:Video,声音:Mic2}
...
function ResourcePill({ resource, label }) {
  ...
  const textOnly=isTextReference(resource)
  const pillLabel=textOnly&&!label.startsWith('[')?`[${label}]`:label
  const mediaKind=resource.type==='声音'?'audio':resource.type==='视频'?'video':'image'
  const mediaSrc=mediaKind==='audio'?demoAudio:mediaKind==='video'?`${import.meta.env.BASE_URL}demo-videos/demo-video-01.mp4`:resource.thumbnail||mockPreviewImages[['主体','人物'].includes(resource.type)?6:resource.type==='场景'?3:4]
  ...
  return <><button ref={trigger} className={`story-resource-pill${textOnly?' is-placeholder':''}`} type="button" onMouseEnter={show} onMouseLeave={scheduleHide} onFocus={show} onBlur={event=>{if(!previewRef.current?.contains(event.relatedTarget))scheduleHide()}} onClick={show} aria-controls={position?id:undefined} aria-expanded={Boolean(position)}>{textOnly?<span className="story-pill-placeholder" aria-hidden="true"><Image/></span>:mediaKind==='image'?<img className="story-pill-thumb" src={mediaSrc} alt=""/>:mediaKind==='video'?<video className="story-pill-thumb" src={mediaSrc} preload="metadata" muted aria-hidden="true"/>:<span className="story-pill-audio" aria-hidden="true">{[4,9,13,7,11].map((height,index)=><i key={index} style={{height}}/>)}</span>}{pillLabel}</button>{position&&createPortal(<div ref={previewRef} id={id} role="dialog" aria-label={`${resource.name}预览`} tabIndex={-1} onFocus={()=>clearTimeout(timer.current)} onBlur={event=>{if(!event.currentTarget.contains(event.relatedTarget)&&event.relatedTarget!==trigger.current)hide()}} className="story-resource-preview" style={{left:position.left,width:position.width,maxHeight:position.maxHeight,...(position.above?{bottom:position.bottom}:{top:position.top})}} onMouseEnter={()=>clearTimeout(timer.current)} onMouseLeave={()=>{if(!previewRef.current?.contains(document.activeElement))scheduleHide()}}><div className={`story-preview-media is-${mediaKind}${textOnly?' is-placeholder':''}`}>{textOnly?<div className="story-preview-placeholder"><Image/><span>待上传图片</span></div>:mediaKind==='image'?<img src={mediaSrc} alt={`${resource.name}示例预览`}/>:mediaKind==='video'?<video src={mediaSrc} controls playsInline preload="metadata"/>:<audio src={mediaSrc} controls preload="metadata"/>}</div><div className="story-preview-caption">{resource.name}</div></div>,document.querySelector('.index-app')||document.body)}</>
}
...
const referenceUses={主体:'参考五官、骨骼形象和体态',人物:'参考五官、形象和体态',服饰:'参考款式、颜色和材质',场景:'仅参考空间、陈设和光线，不采用图中人物',产品:'参考外观、颜色、包装和材质',道具:'参考外观、结构和材质',视频:'参考动作、运镜和节奏',声音:'参考音色、音效和环境声'}
function SegmentResources({ shots, overrides, onSave }) {
  ...
  for(const shot of shots){
    for(const match of shot.text.matchAll(/<(?:主体|服饰)\d+>|\((?:场景|道具)\d+\)|<音效内容：[^>]+>|\(音乐内容：[^)]+\)/g)){
      ...
      if(overrides[key]||storyResources[key])references.set(key,{label:key,resource:overrides[key]||storyResources[key]})
  ...
  return <section className="story-segment-resources" aria-label="当前片段引用资源"><header><strong>引用资源</strong></header><div>{Array.from(references.entries()).map(([key,{label,resource}])=>{const kind=resource.type==='声音'?'audio':resource.type==='视频'?'video':'image';const textOnly=isTextReference(resource);const number=textOnly?null:++counts[kind];const save=(field,value)=>onSave({...overrides,[key]:{...resource,[field]:value}});return textOnly?<div className="story-reference-definition is-text-only" key={key}><ResourcePill resource={resource} label={`[${resource.type}]`}/><span className="story-reference-text-copy"><b><InlineEdit inline label="资源名称" value={resource.name} onSave={value=>save('name',value)}/></b><small><InlineEdit inline label="资源描述" value={resource.description} onSave={value=>save('description',value)}/></small></span></div>:<div className="story-reference-definition" key={key}><ResourcePill resource={resource} label={`@${kind==='audio'?'音频':kind==='video'?'视频':'图片'}${number} · ${label}`}/><span><b><InlineEdit label="附件名称" value={resource.name} onSave={value=>save('name',value)}/></b><small><InlineEdit label="参考用途" value={resource.use??referenceUses[resource.type]} onSave={value=>save('use',value)}/></small></span></div>})}</div></section>
}
...
function InlineEdit({ value, onSave, label, children, className='', inline=false }) {
  ...
  if(draft!==null)return inline?<textarea ref={input} autoFocus rows={1} aria-label={label} className={`story-click-input is-inline ${className}`} value={draft} onChange={event=>setDraft(event.target.value)} onBlur={()=>{onSave(draft);setDraft(null)}} onKeyDown={event=>{if(event.nativeEvent.isComposing)return;if(event.key==='Escape'){event.preventDefault();setDraft(null)}else if(event.key==='Enter'){event.preventDefault();event.currentTarget.blur()}}}/>:<textarea ref={input} autoFocus rows={1} aria-label={label} className={`story-click-input ${className}`} value={draft} onChange={event=>setDraft(event.target.value)} onBlur={()=>{onSave(draft);setDraft(null)}} onKeyDown={event=>{if(event.nativeEvent.isComposing)return;if(event.key==='Escape'){event.preventDefault();setDraft(null)}else if(event.key==='Enter'&&(event.ctrlKey||event.metaKey)){event.preventDefault();event.currentTarget.blur()}}}/>
...
export default function StoryPanel({ settings, onNotice, onValidityChange, demo }) {
  ...
  // Text edits in a segment must not freeze an older attachment binding.
  for(const [key,bound] of Object.entries(demo?.resources||{})) {
    activeResources[key]={...activeResources[key],thumbnail:bound.thumbnail,mediaSrc:bound.mediaSrc}
  }
  ...
  return <div className="story-panel"><div className="segment-settings"><div><Settings2/><span><small>生成模型</small><strong>{settings?.generationModel||'Seedance 2.0-标准版'}</strong></span></div><div><Clock3/><span><small>单段硬上限</small><strong>{settings?.generationModel==='Seedance 2.5'?30:15} 秒</strong></span></div><div><Film/><span><small>智能分段</small><strong>{Math.ceil(shots.length/2)} 个片段</strong></span></div><div><Timer/><span><small>总时长</small><strong>{Math.ceil(shots.length/2)*15} 秒</strong></span></div></div>
    <nav className="story-segment-tabs" aria-label="故事片段">{['一','二','三','四','五'].slice(0,Math.ceil(shots.length/2)).map((name,index)=><button type="button" aria-pressed={segment===index} className={segment===index?'active':''} onClick={()=>setSegment(index)} key={name}><strong>片段{name}</strong><span>15 秒 · 镜头 {index*2+1}–{index*2+2}</span></button>)}</nav>
    <SegmentResources key={`resources-${segment}`} shots={shots.slice(segment*2,segment*2+2)} overrides={activeResources} onSave={value=>setResources(current=>({...current,[segment]:value}))}/>
    <section className="story-segment-content" aria-label={`片段${segment+1}`}>{shots.slice(segment*2,segment*2+2).map((shot,index)=><ShotCard key={segment*2+index} shot={shot} onRegenerate={number=>onNotice?.(`镜头 ${number}：Demo 暂未接入 AI 重新生成服务，已保留当前内容`)} number={segment*2+index+1} overrides={activeResources} onSave={value=>setShots(current=>current.map((item,i)=>i===segment*2+index?value:item))}/>)}</section>
    <ConstraintsEditor key={`constraints-${segment}`} text={constraints[segment]??(demo ? '当前片段 15 秒，两个镜头按时间顺序生成，总片长 60 秒。画面 9:16，保持红蓝瓶外观、原有标签和比例一致；美国成年女性自然 UGC，美式英语口型，约 180 WPM。字幕与台词同步，不承诺恢复排卵，不新增剂量、疗效、折扣或配比。' : defaultConstraints)} onSave={value=>setConstraints(current=>({...current,[segment]:value}))}/>
  </div>
}
