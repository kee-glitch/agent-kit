...
function Drawer({ item, onClose, onPrevious, onNext, onDelete, onSave, notify }) {
  ...
  useEffect(()=>{discardPendingImages();if(aiGenerationTimer.current)window.clearTimeout(aiGenerationTimer.current);aiGenerationTimer.current=null;setAiGenerating(false);setActiveImage(0);setEditing(false);setNameError('');setImageError('');setDraft(createDraft(item))},[item.id])
  useEffect(()=>()=>{discardPendingImages();if(aiGenerationTimer.current)window.clearTimeout(aiGenerationTimer.current)},[])
  const cancelEdit=()=>{discardPendingImages();if(aiGenerationTimer.current)window.clearTimeout(aiGenerationTimer.current);aiGenerationTimer.current=null;setAiGenerating(false);setEditing(false);setNameError('');setImageError('');setActiveImage(0);setDraft(createDraft(item))}
  const generateProductDescription=()=>{setDraft(value=>({...value,description:`${value.name.trim()||'该商品'}商品资产，包含清晰的商品主图、细节展示、包装信息与核心使用场景，可用于快速制作电商详情页及营销素材。`}));notify('AI 已生成商品描述')}
  const generateAudienceMarketing=()=>{if(aiGenerating)return;setAiGenerating(true);aiGenerationTimer.current=window.setTimeout(()=>{setDraft(value=>({...value,coreScenario:'高压崩溃场景：职场高压、长期熬夜、焦虑失眠导致的“过劳肥”与情绪失控。\n生理紊乱场景：月经不调、PCOS、备孕困难、顽固性痤疮与脱发。\n代谢停滞场景：节食无效、体重平台期、嗜糖成瘾、饭后极度困倦。',targetAudience:'核心人群：25-40 岁都市女性，特别是被诊断为 PCOS 或处于亚健康高压状态的职业女性与宝妈。\n心理特征：对传统单一减肥法绝望，深受情绪性进食困扰，渴望通过科学成分从根本上解决身体失控感，追求“身心同调”的安全感。',buyingPointGroups:populationTestGroups.map(group=>({...group}))}));aiGenerationTimer.current=null;setAiGenerating(false);notify('AI 已生成场景、受众与营销组别')},800)}
  const saveEdit=()=>{const name=draft.name.trim();if(!name){setNameError('请输入资产名称');return}const images=draft.images;const buyingPointGroups=draft.buyingPointGroups.map((group,index)=>({...group,group:`营销组别 ${String(index+1).padStart(2,'0')}`,audience:group.audience.trim(),painPoint:group.painPoint.trim(),result:group.result.trim(),feature:group.feature.trim(),benefit:group.benefit.trim(),creative:group.creative.trim()})).filter(group=>group.audience||group.painPoint||group.result||group.feature||group.benefit||group.creative);onSave(item,{name,description:draft.description.trim(),...(item.type==='商品图片'?{coreScenario:draft.coreScenario.trim(),targetAudience:draft.targetAudience.trim(),buyingPointGroups,sellingPoints:buyingPointGroups.map(group=>group.benefit).filter(Boolean),images,imageCount:images.length,previewUrl:images[0]||'',objectImageUrls:[...(item.objectImageUrls||[]),...pendingImageUrls.current]}: {})});pendingImageUrls.current.clear();setEditing(false);setNameError('');setImageError('');notify('更改已保存')}
  const addProductImages=event=>{const files=[...(event.target.files||[])];event.target.value='';setImageError('');if(!files.length)return;const invalid=files.find(file=>!file.type.startsWith('image/'));if(invalid){setImageError('仅支持上传图片文件');return}const oversized=files.find(file=>file.size>50*1024*1024);if(oversized){setImageError('单张图片不能超过 50 MB');return}const urls=files.map(file=>URL.createObjectURL(file));urls.forEach(url=>pendingImageUrls.current.add(url));setDraft(value=>({...value,images:[...value.images,...urls]}));setActiveImage(displayedImages.length);notify(`已添加 ${urls.length} 张图片`)}
  ...
  return <div ref={backdrop} className="asset-detail-overlay" onMouseDown={e=>e.target===e.currentTarget&&close()}>
    <section ref={shell} className={`asset-detail-shell${item.type==='商品图片'?' product-detail-shell':''}`} role="dialog" aria-modal="true" aria-labelledby="asset-detail-title">
      <div className={`asset-detail-media ${item.type==='商品图片'?'product-gallery-media':''}`}><div className="asset-detail-preview" onWheel={handleProductWheel}>{item.type!=='商品图片'?<Thumb item={item} natural playable/>:selectedProductImage?<Thumb item={displayedItem} natural playable/>:<div className="product-image-empty"><Image/><strong>暂无商品图片</strong><span>请从下方上传图片</span></div>}</div>{item.type==='商品图片'&&displayedImages.length>1&&<><button className="product-gallery-arrow previous" onClick={()=>shiftProductImage(-1)} aria-label="上一张商品图"><ArrowLeft/></button><button className="product-gallery-arrow next" onClick={()=>shiftProductImage(1)} aria-label="下一张商品图"><ArrowRight/></button></>}{item.type==='商品图片'&&(displayedImages.length>1||editing)&&<div className={`product-image-strip${editing?' is-editing':''}`} aria-label="商品图片"><span>{displayedImages.length?`${activeImage+1} / ${displayedImages.length}`:'0 / 0'}</span>{displayedImages.map((image,index)=><div className="product-image-item" key={`${image}-${index}`}><button className={`product-image-select ${activeImage===index?'active':''}`} onClick={()=>setActiveImage(index)} aria-label={`查看第 ${index+1} 张商品图`}><img src={image} alt="" loading="lazy" referrerPolicy="no-referrer"/></button>{editing&&<button className="product-image-remove" onClick={()=>removeProductImage(index)} aria-label={`删除第 ${index+1} 张商品图`}><X/></button>}</div>)}{editing&&<label className="product-image-upload"><input type="file" accept="image/*" multiple onChange={addProductImages}/><Upload/><span>上传</span></label>}{imageError&&<small className="product-image-error" role="alert">{imageError}</small>}</div>}</div>
      ...
      <aside className={`asset-detail-copy${editing?' is-editing':''}`}>
        ...
        <div className={`asset-detail-actions${editing?' edit-actions':''}`}>{editing?<><button type="button" onClick={cancelEdit}><X/>取消</button><button type="button" className="save-action" onClick={saveEdit}><Check/>保存更改</button></>:<>{item.type!=='商品图片'&&<><button onClick={()=>notify('资产已开始下载')}><Download/>下载</button><button onClick={()=>notify('已复制资产链接')}><Link2/>复制链接</button></>}{canManage&&item.type==='商品图片'&&<button onClick={()=>setEditing(true)}><Pencil/>编辑</button>}{canManage&&<button className="danger-action" onClick={()=>onDelete(item)}><Trash2/>删除</button>}</>}</div>
      </aside>
    </section>
  </div>
}
...
export default function AssetWorkspace() {
  ...
  return <>
    ...
    <main className="asset-main">
      ...
      {detail&&<Drawer item={detail} onClose={()=>setDetail(null)} onPrevious={()=>showRelative(-1)} onNext={()=>showRelative(1)} onDelete={requestDelete} onSave={(current,patch)=>{setAssetItems(items=>items.map(item=>item.id===current.id?{...item,...patch}:item));setDetail(value=>value?.id===current.id?{...value,...patch}:value)}} notify={notify}/>}
