...
const seedAssets = [
  { id:'asset_0188', name:'品牌光效视觉', description:'用于品牌活动主视觉的方形光影素材。', type:'图片', group:'素材库', scope:'个人资产', source:'用户上传', url:huabanMedia.square, previewUrl:huabanMedia.square, resolution:'2000 × 2000', ratio:'1:1', fileType:'PNG', fileSize:'4.6 MB', tone:'ocean', date:'今天 14:32', creatorId:currentUserId },
  { id:'asset_0187', name:'竖屏动态作品 01', description:'AI 生成的竖屏动态视觉素材。', type:'视频', group:'素材库', scope:'个人资产', source:'用户上传', url:localDemoVideos[0], previewUrl:providedImages[0], ...localVideoSpecs[0], fileType:'MP4', tone:'lime', date:'今天 11:08', creatorId:currentUserId },
  { id:'work_0096', name:'东方美学人物海报', description:'东方人物造型与留白排版结合的竖版视觉提案。', type:'图片作品', group:'作品收藏', scope:'个人资产', source:'从历史记录收藏', url:huabanMedia.posterA, previewUrl:huabanMedia.posterA, taskId:'task_0096', prompt:'竖版东方人物海报，克制留白，强调服装材质与人物轮廓，柔和自然光，高级杂志排版。', skill:'海报视觉生成', model:'Seedream 4.0', ratio:'720:1019', resolution:'1440 × 2038', fileType:'JPG', fileSize:'3.9 MB', tone:'rose', date:'昨天 18:40', creatorId:currentUserId },
  { id:'work_0095', name:'竖屏动态作品 02', description:'根据生成记录收藏的竖屏视频作品。', type:'视频作品', group:'作品收藏', scope:'个人资产', source:'从历史记录收藏', url:localDemoVideos[1], previewUrl:providedImages[1], taskId:'task_0095', prompt:'竖屏构图，主体清晰，镜头自然运动，画面细节稳定。', skill:'文生视频', model:'Seedance 2.0', ...localVideoSpecs[1], fileType:'MP4', tone:'night', date:'昨天 16:22', creatorId:currentUserId },
  { id:'product_0042', name:'便携榨汁杯组合', description:'商品主图、细节图与使用场景图合集。', type:'商品图片', group:'商品资产', scope:'个人资产', source:'商品中心', url:'https://assets.shulan.ai/products/product_0042', images:[huabanMedia.square,huabanMedia.posterA,huabanMedia.posterB,huabanMedia.lightA,huabanMedia.portraitXL,huabanMedia.posterC], previewUrl:huabanMedia.square, imageCount:6, resolution:'2000 × 2000', fileType:'图片集', fileSize:'28.3 MB', tone:'peach', date:'8 月 29 日', creatorId:currentUserId },
  { id:'product_0014', name:'夏季饮品礼盒', description:'包含礼盒主图、包装细节和饮用场景图。', type:'商品图片', group:'商品资产', scope:'个人资产', source:'商品中心', url:'https://assets.shulan.ai/products/product_0014', images:providedImages.slice(2,8), previewUrl:providedImages[2], imageCount:6, resolution:'1152 × 2048', fileType:'图片集', fileSize:'22.6 MB', tone:'peach', date:'8 月 28 日', creatorId:currentUserId },
  { id:'team_0031', name:'品牌视觉规范 2026', description:'团队统一使用的品牌色、字体和版式规范。', type:'图片', group:'素材库', scope:'团队资产', source:'林屿发布', url:huabanMedia.portraitXL, previewUrl:huabanMedia.portraitXL, resolution:'2211 × 2361', ratio:'737:787', fileType:'JPG', fileSize:'5.8 MB', tone:'cobalt', date:'8 月 25 日', creatorId:currentUserId },
  { id:'team_0029', name:'团队竖屏视频 01', description:'团队共享的 AI 竖屏视频素材。', type:'视频', group:'素材库', scope:'团队资产', source:'周一鸣创建', url:localDemoVideos[2], previewUrl:providedImages[2], ...localVideoSpecs[2], fileType:'MP4', tone:'amber', date:'8 月 23 日', creatorId:'user_2048' },
  { id:'team_product_08', name:'团队标准商品图', description:'团队共用的商品白底图、细节图和场景图。', type:'商品图片', group:'商品资产', scope:'团队资产', source:'林屿创建', url:'https://assets.shulan.ai/products/team_product_08', images:[huabanMedia.posterC,huabanMedia.square,huabanMedia.posterA,huabanMedia.lightA,huabanMedia.posterB,huabanMedia.portraitXL,huabanMedia.square,huabanMedia.posterC], previewUrl:huabanMedia.posterC, imageCount:8, resolution:'1200 × 1600', fileType:'图片集', fileSize:'36.1 MB', tone:'peach', date:'8 月 22 日', creatorId:currentUserId },
  { id:'team_product_03', name:'秋季新品组合', description:'团队共享的新品主图、细节图和场景图合集。', type:'商品图片', group:'商品资产', scope:'团队资产', source:'周一鸣创建', url:'https://assets.shulan.ai/products/team_product_03', images:providedImages.slice(6,12), previewUrl:providedImages[6], imageCount:6, resolution:'1152 × 2048', fileType:'图片集', fileSize:'26.8 MB', tone:'peach', date:'8 月 20 日', creatorId:'user_2048' },
]
...
const generatedAssets = Array.from({length:60},(_,index)=>{
  ...
  const source=scope==='团队资产'?(creatorId===currentUserId?'林屿创建':'周一鸣创建'):'用户上传'
...
function Thumb({ item, natural=false, playable=false }) {
  ...
  useEffect(()=>setIntrinsicRatio(null),[item.previewUrl])
  const hideFailedMedia=event=>{event.currentTarget.hidden=true}
  ...
  return <div className={`managed-asset-thumb ${item.tone} ${natural?'natural-ratio':''}`} style={natural?{'--media-ratio':ratio,aspectRatio:ratio}:undefined}><div className="managed-thumb-art"/>{item.previewUrl&&<img src={item.previewUrl} alt="" loading={natural?'eager':'lazy'} referrerPolicy="no-referrer" onLoad={readImageRatio} onError={hideFailedMedia}/>} {useEmbeddedPlayer?<a className="video-external-preview" href={item.url} target="_blank" rel="noreferrer"><ExternalLink/>新窗口播放</a>:playable&&isVideo(item)&&<video src={item.url} poster={item.previewUrl} controls playsInline preload="metadata" referrerPolicy="no-referrer" onError={hideFailedMedia}/>} {isVideo(item)&&!playable&&<i>▶</i>}</div>
}

function Sidebar({ scope, setScope, group, setGroup }) {
  const groups = scope === '个人资产' ? [['素材库',FolderOpen],['作品收藏',GalleryVerticalEnd],['商品资产',Package]] : [['素材库',FolderOpen],['商品资产',Package]]
  const count = name => assets.filter(item => item.scope===scope && (name==='全部资产'||item.group===name)).length
...
function DetailRows({ item }) {
  const rows = item.type==='营销报告' ? [['页数',`${item.pages} 页`],['文件类型',item.fileType],['文件大小',item.fileSize]] : item.type==='商品图片' ? [['图片数量',`${item.imageCount} 张`],['分辨率',item.resolution],['文件类型',item.fileType],['文件大小',item.fileSize]] : isVideo(item) ? [['画面比例',item.ratio],['清晰度',item.clarity],['时长',item.duration],['文件类型',item.fileType],['文件大小',item.fileSize]] : [['画面比例',item.ratio],['分辨率',item.resolution],['文件类型',item.fileType],['文件大小',item.fileSize]].filter(([,value])=>value)
  return <>{rows.map(([label,value])=><div key={label}><dt>{label}</dt><dd>{value}</dd></div>)}</>
}

function Drawer({ item, onClose, onPrevious, onNext, notify }) {
  ...
  useLayoutEffect(()=>{const mm=gsap.matchMedia();mm.add('(prefers-reduced-motion: no-preference)',()=>{gsap.fromTo(backdrop.current,{autoAlpha:0},{autoAlpha:1,duration:.2});gsap.fromTo(shell.current,{autoAlpha:0,scale:.99},{autoAlpha:1,scale:1,duration:.3,ease:'power2.out',clearProps:'transform'})});return()=>mm.revert()},[])
  useEffect(()=>{const key=e=>{if(e.key==='Escape')close();if(e.key==='ArrowUp')onPrevious();if(e.key==='ArrowDown')onNext()};document.addEventListener('keydown',key);closeButton.current?.focus();return()=>document.removeEventListener('keydown',key)},[onPrevious,onNext])
  ...
  return <div ref={backdrop} className="asset-detail-overlay" onMouseDown={e=>e.target===e.currentTarget&&close()}>
    <section ref={shell} className="asset-detail-shell" role="dialog" aria-modal="true" aria-labelledby="asset-detail-title">
      ...
      <aside className="asset-detail-copy"><header><span>{item.group} / {item.type}</span><h2 id="asset-detail-title">{item.name}</h2><p>{item.description}</p></header><div className="asset-detail-copy-scroll"><section className="detail-section"><h3>基础信息</h3><dl><div><dt>资产 ID</dt><dd><code>{item.id}</code></dd></div><div><dt>URL</dt><dd className="detail-url">{item.url}</dd></div><div><dt>来源</dt><dd>{item.source}</dd></div><div><dt>归属</dt><dd>{item.scope}</dd></div></dl></section>{item.group==='作品收藏'&&<section className="detail-section"><h3>关联与生成信息</h3><dl><div><dt>任务 ID</dt><dd><code>{item.taskId}</code></dd></div><div><dt>模型</dt><dd>{item.model}</dd></div><div><dt>技能</dt><dd>{item.skill}</dd></div><div className="detail-prompt"><dt>提示词</dt><dd>{item.prompt}</dd></div></dl></section>}<section className="detail-section"><h3>{item.type==='营销报告'?'报告属性':'媒体属性'}</h3><dl><DetailRows item={item}/></dl></section>{item.scope==='团队资产'&&<section className="detail-section"><h3>成员权限</h3><dl><div><dt>查看与创建</dt><dd>团队成员</dd></div><div><dt>修改与删除</dt><dd>{canManage?'你是创建者':'仅创建者'}</dd></div></dl></section>}</div><div className="asset-detail-actions"><button onClick={()=>notify('资产已开始下载')}><Download/>下载</button><button onClick={()=>notify('已复制资产链接')}><Link2/>复制链接</button>{canManage&&<button onClick={()=>notify('已进入编辑状态')}><Pencil/>编辑</button>}{canManage&&<button className="danger-action" onClick={()=>notify('删除操作需要二次确认')}><Trash2/>删除</button>}</div></aside>
    </section>
  </div>
}
...
function AssetCard({ item, selected, onOpen, onToggle, notify }) {
  return <article className={`asset-card ${selected?'selected':''}`}>
    ...
    <div className="asset-info compact"><strong>{item.name}</strong><p><span>{item.source}</span><time>{item.date}</time></p></div>
  </article>
}

export default function AssetWorkspace() {
  const [scope,setScope]=useState('个人资产'),[group,setGroup]=useState('全部资产'),[query,setQuery]=useState(''),[type,setType]=useState('全部类型'),[view,setView]=useState('grid'),[selected,setSelected]=useState([]),[detail,setDetail]=useState(null),[upload,setUpload]=useState(false),[uploadFile,setUploadFile]=useState(''),[uploadError,setUploadError]=useState(''),[uploading,setUploading]=useState(false),[toast,setToast]=useState('')
  const filtered=useMemo(()=>assets.filter(item=>item.scope===scope&&(group==='全部资产'||item.group===group)&&(type==='全部类型'||item.type===type)&&(item.name.includes(query)||item.id.includes(query))),[scope,group,type,query])
  ...
  const canDeleteSelection=selected.every(id=>assets.find(item=>item.id===id)?.creatorId===currentUserId)
  ...
  const chooseFile=event=>{const file=event.target.files?.[0];setUploadError('');setUploadFile('');if(!file)return;if(!file.type.startsWith('image/')&&!file.type.startsWith('video/'))return setUploadError('仅支持图片或视频文件。');if(file.size>2*1024*1024*1024)return setUploadError('文件超过 2 GB，请压缩后重试。');setUploadFile(file.name)}
  const submitUpload=()=>{if(!uploadFile||uploading)return;setUploading(true);window.setTimeout(()=>{setUploading(false);setUpload(false);setUploadFile('');notify('资产上传成功')},700)}
  const closeUpload=()=>{if(uploading)return;setUpload(false);setUploadFile('');setUploadError('')}

  return <>
    <Sidebar {...{scope,setScope,group,setGroup}}/>
    <main className="asset-main">
      ...
      {selected.length>0&&<div className="selection-bar"><span><b>{selected.length}</b> 项已选择</span>{scope==='个人资产'&&<button onClick={()=>notify('已在团队资产中创建副本，并保留来源关联')}><Users/>发布到团队</button>}{canDeleteSelection&&<button onClick={()=>{setSelected([]);notify('所选资产已移至回收站')}}><Trash2/>删除</button>}<button className="selection-close" onClick={()=>setSelected([])} aria-label="取消选择"><X/></button></div>}
      {detail&&<Drawer item={detail} onClose={()=>setDetail(null)} onPrevious={()=>showRelative(-1)} onNext={()=>showRelative(1)} notify={notify}/>} 
      {upload&&<div className="dialog-backdrop" onMouseDown={e=>e.target===e.currentTarget&&closeUpload()}><section className="upload-dialog" role="dialog" aria-modal="true" aria-labelledby="upload-title"><header><div><h2 id="upload-title">上传到素材库</h2><p>支持图片与视频，上传后遵循当前资产归属权限。</p></div><button onClick={closeUpload} disabled={uploading} aria-label="关闭上传窗口"><X/></button></header><label className={`upload-drop ${uploadError?'error':''}`}><input type="file" accept="image/*,video/*" onChange={chooseFile}/><Upload/><strong>{uploadFile||'选择文件上传'}</strong><span>{uploadError||'图片最大 50 MB，视频最大 2 GB'}</span></label><footer><button className="cancel" onClick={closeUpload} disabled={uploading}>取消</button><button className="confirm" onClick={submitUpload} disabled={!uploadFile||uploading}>{uploading?'上传中…':'继续'}</button></footer></section></div>}
